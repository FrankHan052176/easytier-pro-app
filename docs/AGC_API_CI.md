...
The update request refuses to proceed without at least one group. It writes every `groupId`, a start time one hour after the current UTC time, an end time after `AGC_TEST_DURATION_DAYS`, `displayArea="1"`, `needShareLink=0`, and `needNotify` (`0` for silent, `1` to send notifications). The workflow defaults to silent releases (`needNotify=0` for Core repository dispatches and push runs unless `AGC_NOTIFY_ON_PUSH=1`). The `workflow_dispatch` trigger provides a `notify` checkbox option (defaulting to false / silent) to explicitly toggle invitation notifications when testing or triggering manually.
